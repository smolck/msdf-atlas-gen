...
bool generateShadronPreview(const FontGeometry *fonts, int fontCount,
  ...
  if (!file)
    return false;
  fprintf(file, shadronPreviewPreamble,
          atlasType == ImageType::HARD_MASK || atlasType == ImageType::SOFT_MASK
              ? shadronFillGlyphMask
              : shadronFillGlyphSdf);
  if (imageFilename)
    fprintf(file, "image Atlas = file(\"%s\")",
            relativizePath(outputFilename, imageFilename).c_str());
  else
          ...
          pxRange * texelWidth, pxRange * texelHeight);
  {
    ...
    for (int i = 1; i < fontCount; ++i) {
      fontMetrics.lineHeight =
          std::max(fontMetrics.lineHeight, fonts[i].getMetrics().lineHeight);
      fontMetrics.ascenderY =
          std::max(fontMetrics.ascenderY, fonts[i].getMetrics().ascenderY);
      fontMetrics.descenderY =
          std::min(fontMetrics.descenderY, fonts[i].getMetrics().descenderY);
    }
      ...
      for (int i = 0; i < fontCount; ++i) {
        ...
        if (glyph) {
          if (!glyph->isWhitespace()) {
            ...
            pl += x, pb += y, pr += x, pt += y;
            il *= texelWidth, ib *= texelHeight, ir *= texelWidth,
                it *= texelHeight;
            fprintf(file,
                    "    %.9g, %.9g, %.9g, %.9g, %.9g, %.9g, %.9g, %.9g, %.9g, "
                    "%.9g, %.9g, %.9g, %.9g, %.9g, %.9g, %.9g, %.9g, %.9g, "
                    "%.9g, %.9g, %.9g, %.9g, %.9g, %.9g,\n",
                    pl, pb, il, ib, pr, pb, ir, ib, pl, pt, il, it, pr, pt, ir,
                    it, pl, pt, il, it, pr, pb, ir, ib);
          }
  ...
  fputs("PREVIEW_IMAGE(Preview, Atlas, txRange, vec3(1.0), textQuadVertices, "
        "textSize, ivec2(1200, 400));\n",
        file);
