...
static void copyRectanglePlacement(OrientedRectangle &dst,
                                   const OrientedRectangle &src) {
  dst.x = src.x;
  dst.y = src.y;
...
int packRectangles(RectangleType *rectangles, int count, int width, int height,
  ...
  int result = RectanglePacker(width + padding, height + padding)
                   .pack(rectangles, count);
  if (padding)
    for (int i = 0; i < count; ++i) {
      rectangles[i].w -= padding;
      rectangles[i].h -= padding;
...
std::pair<int, int> packRectangles(RectangleType *rectangles, int count,
  ...
  while (sizeSelector(width, height)) {
    if (!RectanglePacker(width + padding, height + padding)
             .pack(rectanglesCopy.data(), count)) {
