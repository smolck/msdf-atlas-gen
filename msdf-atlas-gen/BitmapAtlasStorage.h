...
/// An implementation of AtlasStorage represented by a bitmap in memory
/// (msdfgen::Bitmap)
...
public:
  ...
  BitmapAtlasStorage(const BitmapAtlasStorage<T, N> &orig, int width,
                     int height);
  BitmapAtlasStorage(const BitmapAtlasStorage<T, N> &orig, int width,
                     int height, const Remap *remapping, int count);
