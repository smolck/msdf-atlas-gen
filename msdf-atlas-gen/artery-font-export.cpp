  ...
  for (int i = 0; i < fontCount; ++i) {
    ...
    case GlyphIdentifierType::GLYPH_INDEX:
      for (const std::pair<std::pair<int, int>, double> &elem :
        ...
        fontVariant.kernPairs.vector.push_back((artery_font::KernPair<REAL> &&)
                                                   kernPair);
      }
      break;
    case GlyphIdentifierType::UNICODE_CODEPOINT:
      for (const std::pair<std::pair<int, int>, double> &elem :
        ...
        if (glyph1 && glyph2 && glyph1->getCodepoint() &&
          ...
          kernPair.advance.h = REAL(elem.second);
          fontVariant.kernPairs.vector.push_back(
              (artery_font::KernPair<REAL> &&) kernPair);
        }
      }
      break;
    }
    arfont.variants.vector.push_back((artery_font::StdFontVariant<REAL> &&)
                                         fontVariant);
  }
