...
void blitSameType(const msdfgen::BitmapRef<T, N> &dst,
                  const msdfgen::BitmapConstRef<T, N> &src, int dx, int dy,
                  int sx, int sy, int w, int h) {
  for (int y = 0; y < h; ++y)
    memcpy(dst(dx, dy + y), src(sx, sy + y), sizeof(T) * N * w);
}

#define BLIT_SAME_TYPE_IMPL(T, N)                                              \
  void blit(const msdfgen::BitmapRef<T, N> &dst,                               \
            const msdfgen::BitmapConstRef<T, N> &src, int dx, int dy, int sx,  \
            int sy, int w, int h) {                                            \
    blitSameType(dst, src, dx, dy, sx, sy, w, h);                              \
  }
