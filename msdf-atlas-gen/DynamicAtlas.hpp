...
DynamicAtlas<AtlasGenerator>::DynamicAtlas()
    : glyphCount(0), side(0), totalArea(0), padding(0) {}

template <class AtlasGenerator>
DynamicAtlas<AtlasGenerator>::DynamicAtlas(AtlasGenerator &&generator)
    : generator((AtlasGenerator &&) generator), glyphCount(0), side(0),
      totalArea(0), padding(0) {}
...
void DynamicAtlas<AtlasGenerator>::add(GlyphGeometry *glyphs, int count) {
  ...
  if ((int)rectangles.size() > start) {
    ...
    while (packer.pack(rectangles.data() + packerStart,
                       rectangles.size() - packerStart) > 0) {
      side = side + !side << 1;
    ...
    for (int i = start; i < (int)rectangles.size(); ++i) {
      ...
      glyphs[remapBuffer[i].index - glyphCount].placeBox(rectangles[i].x,
                                                         rectangles[i].y);
    }
  }
